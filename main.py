...
from agents import AgentFinancialAnalyst, AgentFinancialAnalystWithHistory
from chains import FinancialAnalysisChainOneShot, FinancialAnalysisChainWithHistory
...
def parse_answer(answer):
    ...
    pattern2 = re.compile(r"(\d+(\.\d+)?|\d+)%")  # "e.g. from operating activities was 14.13%."
...
def generate_reports(model, description, do_html=True, do_csv=True):
    """
    ...
    if do_html:
        generate_html_report(
            filename_html,
            header={"model": model, "description": description},
            data=results,
        )
    if do_csv:
...
if __name__ == "__main__":
    ...
    # choose between BedrockStrategy or OpenAIStrategy
    llm = OpenAIStrategy
    # choose between FinancialAnalysisChainOneShot or FinancialAnalysisChainWithHistory
    chain = FinancialAnalysisChainWithHistory
    # choose between AgentFinancialAnalyst or AgentFinancialAnalystWithHistory
    agent = AgentFinancialAnalystWithHistory(llm=llm, chain=chain)
    ...
    generate_reports(model=agent.model_name, description=agent.chain_description)
